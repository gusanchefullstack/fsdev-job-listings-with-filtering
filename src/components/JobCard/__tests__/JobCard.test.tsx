...
describe('JobCard', () => {
  it('renders company name, position, contract, location and postedAt', () => {
    render(<JobCard job={baseJob} activeFilters={[]} onAddFilter={vi.fn()} />)
  ...
  it('does NOT render New! badge when job.new is false', () => {
    render(<JobCard job={{ ...baseJob, new: false }} activeFilters={[]} onAddFilter={vi.fn()} />)
    expect(screen.queryByText('New!')).not.toBeInTheDocument()
  })

  it('renders New! badge when job.new is true', () => {
    render(<JobCard job={{ ...baseJob, new: true }} activeFilters={[]} onAddFilter={vi.fn()} />)
    expect(screen.getByText('New!')).toBeInTheDocument()
  })

  it('does NOT render Featured badge when job.featured is false', () => {
    render(<JobCard job={{ ...baseJob, featured: false }} activeFilters={[]} onAddFilter={vi.fn()} />)
    expect(screen.queryByText('Featured')).not.toBeInTheDocument()
  })

  it('renders Featured badge when job.featured is true', () => {
    render(<JobCard job={{ ...baseJob, featured: true }} activeFilters={[]} onAddFilter={vi.fn()} />)
  ...
  it('renders all tags from role, level, languages and tools', () => {
    ...
    render(<JobCard job={job} activeFilters={[]} onAddFilter={vi.fn()} />)
  ...
  it('clicking a tag calls onAddFilter with the correct tag', async () => {
    ...
    render(<JobCard job={baseJob} activeFilters={[]} onAddFilter={onAddFilter} />)
  ...
  it('active filter tags have aria-pressed="true"', () => {
    render(<JobCard job={baseJob} activeFilters={['Frontend']} onAddFilter={vi.fn()} />)
    expect(screen.getByRole('button', { name: /filter by frontend/i })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: /filter by senior/i })).toHaveAttribute('aria-pressed', 'false')
  })
})
